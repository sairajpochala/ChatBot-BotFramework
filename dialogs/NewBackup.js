...
.matches('Love me',(session,args)=>{
 ...
 .matches('Yes',(session,args)=>{
    ...
    if (FlagAlarm == true){
         ...
         session.userData['pendingRequest'] = true;
         session.endDialog(); 
    }
    if (FlagpendingRequest == true){
...
.matches('No',(session,args)=>{
    ...
    if (FlagpendingRequest == true){
        ...
        // session.send("The node has a hardware failure");
         console.log("Opens the Order details page");
         session.userData['OrdersPage'] = false;
         session.userData['case1B'] = true; // done with case
    ...
    // }
         ...
         session.endDialog("My Pleasure 2");
...
bot.dialog('Orders',[
    ...
    function(session, args, next){
     ...
     if(FlagprovisionYes == true){
         ...
         session.send("No, We do not have the capacity to provision this request.");
         session.userData['dashboard'] = true;
     }},
    ...
    function(session, args, next){
        session.send("Open Stack cluster");
...
bot.on("event", function (event) {
    ...
    if (event.name === "buttonClicked") {
      ...
      pendingRequest = event.value['pendingRequest'];
        
       FlagpendingRequest = true;
        
        msg.text(pendingRequest);  
        bot.send(msg);
        console.log(pendingRequest);
      // bot.beginDialog(event.address, 'Question');
    }
