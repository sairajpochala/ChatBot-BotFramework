...
//module.exports = [
        ...
        /* check for project entent
        check for wrongly spelled services  Flag1 if present
        check for users  Flag2 if not present            
        check for dates  Flag3 if not present     
        check for services    Flag4 if not present
        
        if flag 1 present
        ask question or next  -- if he says yes then next
        
        if flag 2 is present
        

        s
        if there then next else execute and then next
    ...
    //////////////////////////////
     ...
     async function(session, result, next){
        ...
        if(result.response){
            ...
            // if(result.response == next({ resumed: builder.ResumeReason.back }) )
            //     next({ resumed: builder.ResumeReason.back }) 
            // else{    
            ...
            //session.send(args.entities);
            console.log("003A"); console.log("1");
console.log(next({ resumed: builder.ResumeReason.back }));
            console.log("004");console.log(args.entities);console.log("2"+ typeof args);
             console.log(args.entities.length);
                if (args.entities.length> 0){ 
            
                    session.userData['Services'] = args.entities;
                    console.log(args.entities.resolution.values[1].Start);
                    console.log("005");
                    FinishEntity = builder.EntityRecognizer.findEntity(args.entities, 'Finish');
                    WrongServiceEntity = builder.EntityRecognizer.findEntity(args.entities, 'WrongService');
