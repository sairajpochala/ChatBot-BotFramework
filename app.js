...
var FlagG = false;
var DecisionT=false;
var FlagThanks = false;
var greetServices;
var StartDay,EndDay,StartYear,StartYearT,EndYearT,EndYear,StartMonth,EndMonth,CurrentDay,CurrentMonth,CurrentYear;
// /*-----------------------------------------------------------------------------
...
var regExThx = new builder.RegExpRecognizer('Thank You', /.*thank( you|s)|thanks|great.*/i); 
...
var regExNo = new builder.RegExpRecognizer('No', /.*no|no need|no thanks|Okie Thanks, I will check that node||Thanks, I will check that node|Thanks,I will check that note|na.*/i);
var regExYes = new builder.RegExpRecognizer('Yes', /.*yes|ok|okay|k|yup|sure|yes I would like to see them|yes that would help me|yes i would like to see them|yes it would be helpful|yes that would be great|YES|ya.*/i);
    
var intents = new builder.IntentDialog({ recognizers:[recognizer,regExNo,regExYes,regExJoke,regExHwRU,regExWhoRU,regExWhoAmI,regWSup,regILU,regULM, regExgoodBye]})     
...
.matches('Love me',(session,args)=>{
 ...
 .matches('Yes',(session,args)=>{
    ...
    session.userData['servicespage1'] = false;
    ...
    if (FlagG == true){
         session.userData['servicespage1'] = true;
         session.send("Opening Service page.. ");
    ...
    // }
         ...
         session.endDialog("My Pleasure,How can I help you further?");
...
//session.userData['provisionCheck'] = false;
    if (FlagG == true){
         ...
         session.endDialog("Welcome, How can I help you further?"); 
...
// Create a new request for allen for Project 1 from 8 august to 10 september with Gitlab and gerrit as services
 ...
 //////////////////////////////////////////////////////               ////////////////// code  ////////////////////////////////////////
         ...
         date_invalid2 = false;
         console.log("today date1",currentdate);
         
         var startdate = new Date(args.entities[0].resolution.values[0].start); 
         var enddate = new Date(args.entities[0].resolution.values[0].end); 

        console.log("startFirst",startdate);
        console.log("endFirst",enddate);
        console.log("currentFirst",currentdate);
        
        StartDay  = startdate.getDate();
        StartMonth= startdate.getMonth();
        StartYearT = startdate.getFullYear(); 
            if (StartYearT<"2019"){
                StartYear= "2019";
                console.log("StartYearsai1",StartYear);
            }else{
                StartYear= StartYearT;
                console.log("StartYearsai2",StartYear);
            };
        
        EndDay  = enddate.getDate();
        EndMonth= enddate.getMonth();
        EndYearT = enddate.getFullYear();
      
       if (EndYearT<"2019"){
                EndYear= "2019";
                console.log("EndYearsai1",EndYear);
            }else{
                EndYear= StartYearT;
                console.log("EndYearsai2",EndYear);
            };
        ...
        if (EndYear < StartYear) date_invalid2 = true ,console.log("date invalid 1");
        else if ((EndYear == StartYear) && (( StartMonth - EndMonth )> 0))  date_invalid2 = true, console.log("date invalid 2");
        else if ((EndYear == StartYear) &&( EndMonth > StartMonth)&& ((StartDay - EndDay) > 0 ))date_invalid2 = true , console.log("date invalid 3");
        
        else if (CurrentYear > StartYear) date_invalid2 = true ,console.log("date invalid 4");
        else if ((CurrentYear == StartYear) && (( CurrentMonth - StartMonth   )> 0))  date_invalid2 = true, console.log("date invalid 5");
        else if ((CurrentYear == StartYear) &&( CurrentMonth == StartMonth)&& ((CurrentDay- StartDay) > 0) )date_invalid2 = true , console.log("date invalid 6");
                
       console.log("date invalid 2",date_invalid2 );
  
     //  if  (date_invalid2 == true) {     next({ resumed: builder.ResumeReason.back, reason: 'wrong' });
 ////////////////////////////////////////////               /////////////////////////////////// here ////////////////////////////////
       ...
       function(session,result, next){
        ...
        if (date_invalid2 == true) {
            
            console.log("came here here");
            date_invalid2 = false;
        builder.Prompts.text(session, "The mentioned date is not a valid one. Please select a valid start and an end date");
        }  
    ...
    async function(session, result, next){
       ...
       // console.log("today date2", date.toLocaleDateString());
        ...
        if (EndYear < StartYear) date_invalid = true ,console.log("date invalid 1");
        else if ((EndYear == StartYear) && (( StartMonth - EndMonth )> 0))  date_invalid = true, console.log("date invalid 2");
        else if ((EndYear == StartYear) &&( EndMonth > StartMonth)&& ((StartDay - EndDay) > 0 ))date_invalid = true , console.log("date invalid 3");
        
        else if (CurrentYear > StartYear) date_invalid = true ,console.log("date invalid 4");
        else if ((CurrentYear == StartYear) && (( CurrentMonth - StartMonth   )> 0))  date_invalid = true, console.log("date invalid 5");
        else if ((CurrentYear == StartYear) &&( CurrentMonth == StartMonth)&& ((CurrentDay- StartDay) > 0) )date_invalid = true , console.log("date invalid 6");
